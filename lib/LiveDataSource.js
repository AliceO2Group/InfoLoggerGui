...
module.exports = class LiveDataSource extends EventEmitter {
  ...
  parse(trame) {
    ...
    if (!trameProtocol) {
      ...
      log.warn(`Parsing: unreconized protocol, found "${trameVersion}", support ${protocolsVersions.join(', ')}`);
      return;
    }
    ...
    if (fields.length !== trameProtocol.fields.length) {
      log.warn(`Parsing: expected ${trameProtocol.fields.length} fields for protocol version ${trameProtocol.version}, found ${fields.length}`);
      return;
    }
