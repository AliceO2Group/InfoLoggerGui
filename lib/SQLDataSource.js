...
module.exports = class SQLDataSource {
  constructor(options) {
    ...
    this.state = 'Not connected'; // contains 'connected' or any message why it is not
  ...
  errorHandler(err) {
    ...
    if (err.code === 'ER_NO_DB_ERROR') {
      this.state = `Unable to connect to mysql, ${this.options.database} database not found`;
      log.warn(this.state);
    } else if (err.code === 'ER_NO_SUCH_TABLE') {
      this.state = `Unable to connect to mysql, "messages" table not found in ${this.options.database}`;
      log.warn(this.state);
    } else if (err.code === 'ETIMEDOUT' || err.code === 'ECONNREFUSED') {
      this.state = `Unable to connect to mysql on ${this.options.host}:${this.options.port}`;
      log.warn(this.state);
    } else if (err.code === 'ER_DBACCESS_DENIED_ERROR') {
      this.state = `Unable to connect to mysql, access denied for ${this.options.user}`;
      log.warn(this.state);
    } else {
      this.state = `Unable to connect to mysql: ${err.code}`;
