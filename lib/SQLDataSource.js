...
module.exports = class SQLDataSource {
  ...
  queryFromFilters(filters, options, callback) {
    ...
    */
    ...
    // The user need this info to give an order of idea
    // But for performance issue, we need to limit so block at 100001 and show "100000+"
    const total = new Promise((resolve, reject) => {
      const request = `SELECT COUNT(*) as total FROM \`messages\` ${criteriasString} LIMIT 100001`;
